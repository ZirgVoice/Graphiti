...
class StarWarsIntrospectionTests: XCTestCase {
    ...
    func testIntrospectionDefaultValue() throws {
        let introspection = try api.execute(
            request: """
        ...
        XCTAssertEqual(
            introspection.description.contains("Cannot convert value to AST: 0"),
            false
        )
        
        XCTAssertEqual(
            introspection.description.contains("Cannot convert value to AST: 1"),
            false
        )
    }
}
